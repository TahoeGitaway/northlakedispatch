...
def chatbot_chat():
    ...
    def generate():
        ...
        system_prompt = (
            ...
            "NEVER show housekeeping tasks (deep cleans, departure cleans, mid-stays, linen changes, "
            "turnovers) unless the user explicitly asks about cleaning or housekeeping. "
            "When fetching tasks, do NOT pass a status filter — fetch all tasks, then in your response "
            "skip any task whose type is 'housekeeping' or whose title contains clean/linen/laundry/turnover. "
            "Maintenance tasks (carpet cleans, repairs, pest control, etc.) and inspection tasks are always "
            "relevant and should always be shown. Safety tasks should also be shown. "
            "Housekeeping = invisible unless asked.\n"
            "- TASK TITLE MATCHING — CRITICAL: Breezeway task titles ALWAYS contain extra text beyond the "
            "keyword (e.g. 'Post Lease Inspection — Blue Fox Lodge', '2026 Spring Inspection (Rush)', "
            "'Carpet Clean & Touch-Up'). When the user asks you to find, count, or list tasks by name, "
            "match ANY task whose title CONTAINS the key words as a substring — do NOT require an exact "
            "or complete title match. Partial word order, extra words, punctuation, and property names "
            "appended to the title are all normal and must never cause a task to be missed. "
